...
export DIR_SCRIPTS="/app/startup_scripts"
export DIR_CONFIG="/app/configs"
export DIR_SHARED_CONFIG="/etc/nginx/autoconf"


# Files 
export FILE_NGINX_CONF="$DIR_CONFIG/nginx.store.conf"
export FILE_NGINX_CONF_STORE="$DIR_SHARED_CONFIG/nginx.${HOSTNAME}.conf"
...
if [ -z "$DB_RESET" ]; then
    echo "DB_RESET is not set. Setting REMIGRATE to false..."
    DB_RESET=false
fi
