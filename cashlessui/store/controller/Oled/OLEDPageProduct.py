...
class OLEDPageProduct(OLEDPage):
    ...
    def view(self, product: StoreProduct, *args, **kwargs):
        ...
        if product.discount > 0:
            draw.text((30, content_y_start), f"SALE {product.get_final_price()}", font=self.font_large,fill=(255,255,255))
            draw.text((30, content_y_start + 25), f"EUR {product.resell_price}", font=self.font_large,fill=(255,255,255))
            draw.text((30, content_y_start + 37), f"Stock {product.stock_quantity}", font=self.font_large,fill=(255,255,255))
        else:
            draw.text((30, content_y_start), f"EUR {product.get_final_price()}", font=self.font_large,fill=(255,255,255))
            draw.text((30, content_y_start + 25), f"Stock {product.stock_quantity}", font=self.font_large,fill=(255,255,255))
            draw.text((30, content_y_start + 37), f"Press A to continue or place NFC to buy", font=self.font_regular, fill=(255,255,255))
