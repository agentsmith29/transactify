...
def extract_svg(zip_ref, file_name):
    ...
    svg_path = os.path.join(SVG_DIR, os.path.basename(file_name))
...
def process_png(ref, file_name):
    file_name = str(os.path.basename(file_name)).replace('.svg', '.png')
    print(f"Processing PNG: {file_name}")
    with open(ref, "rb") as file:
        # convert the svg file to png
        for size, dir_path in PNG_DIRS.items():
            png_filename = os.path.join(dir_path, file_name)
            #, format="PNG"
